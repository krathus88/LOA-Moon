...
type RowHeaderPartyBuffsBreakdownProps = {
    keyValue: string;
    synergy: EncounterSynergyDataType;
};

export function RowHeaderPartyBuffsBreakdown({
    keyValue,
    synergy,
}: RowHeaderPartyBuffsBreakdownProps) {
    ...
    return (
        <th className={`text-center ${keyValue}`}>
            <img
                className={`rounded tt-s-${synergy.type}`}
                src={iconVariants[currentIconIndex]}
                alt={`${synergy.type}`}
                onError={handleImgError} // On error, try next variant
            />
            <ItemTooltip anchorTo={`.${keyValue} .tt-s-${synergy.type}`}>
